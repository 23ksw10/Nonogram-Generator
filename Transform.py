...
def getBackgroundColor(img):
    ...
    while len(candidateClrs) > 0:
        h = candidateClrs[0]
        if clrs[h] >= img.size // 20 and clrsBoundary[h] >= 2 * (width + height) // 10 and clrsMiddle[
            h] <= img.size // 4:
            break
        candidateClrs.pop(0)
...
def drawGrid(clues, imgShape, bgColor, SQUARESIZE=40):
    ...
    img = np.full(
        ((maxVClues + height) * SQUARESIZE + height + 1, (maxHClues + width) * SQUARESIZE + width + 1, len(bgColor)),
        255)
    img[maxVClues * SQUARESIZE:, maxHClues * SQUARESIZE:] = bgColor
    img[:, [maxHClues * SQUARESIZE + (SQUARESIZE + 1) * k for k in range(width + 1)]] = (0, 0, 0) if sum(
        x for x in bgColor) >= 30 else (100, 100, 100)
    img[[maxVClues * SQUARESIZE + (SQUARESIZE + 1) * k for k in range(height + 1)], :] = (0, 0, 0) if sum(
        x for x in bgColor) >= 30 else (100, 100, 100)
    return img
...
def drawClues(img, clues, imgShape):
    ...
    for i in range(len(hClues)):
        ...
        for j in range(len(hClues[i])):
            ...
            cv2.putText(img, str(hClues[i][j][1]), (startX + xOffset, startY + 4 * SQUARESIZE // 5),
                        cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=compClr, thickness=2)
            startX += SQUARESIZE
    for i in range(len(vClues)):
        ...
        for j in range(len(vClues[i])):
            ...
            img[startY:startY + SQUARESIZE, startX:startX + SQUARESIZE] = clr
            cv2.putText(img, str(vClues[i][j][1]), (startX + xOffset, startY + 4 * SQUARESIZE // 5),
                        cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=compClr, thickness=2)
            startY += SQUARESIZE
    return img
